...
| Gate | Status | Notes |
|------|--------|-------|
| Gate 1: Verilator sim | **PASS** (2026-03-24) | CPU boots, 2M+ AS cycles / 10 frames, not halted; video is test-pattern stub |
| Gate 2: RTL lint | **PASS** (2026-03-24) | check_rtl.sh 0 FAILs; 1 harmless WARN (cen port) |
| Gate 3: Standalone synthesis | READY | standalone_synth/ scaffolded; run `quartus_sh --flow compile standalone.qsf` |
| Gate 4: Full system synthesis | NOT STARTED | quartus/ has SDC; full emu.sv not yet written |
...
**Blocking issues:**
1. BAC06 tile generator RTL — critical path; jtcores/cop/hdl/jtcop_bac06.v is community reference
...
- BAC06 RTL from jtcop_bac06.v community reference (jtcores/cores/cop/hdl/)
- Standalone synthesis run (Gate 3)

**Ecosystem note (2026-03-24):**
jotego covers ALL 9 dec0.cpp games: jtcop (robocop+hippodrm), jtninja (baddudes+hbarrel),
jtmidres (midres), jtslyspy (secretag+bouldash). Games birdtry and bandit are minor/rare.
The uncovered territory is the **cninja.cpp family**: Caveman Ninja, Tumble Pop, Robocop 2,
Joe & Mac Returns, Funky Jet, Nitro Ball — all use DECO16IC chip (4-plane BG + sprites).
ROMs confirmed on rpmini: cninja.zip, tumblep.zip, robocop2.zip, joemacr.zip, funkyjet.zip, nitrobal.zip.
Those require a separate **deco16_cninja** core with DECO16IC RTL.
