...
module v60_core (
    ...
    // =========================================================================
    // Opcode decode sub-modules (split by opcode[7:6] to reduce Quartus memory)
    // Each sub-module is pure combinational; the mux selects the active bundle.
    // =========================================================================
    v60_decode_t dec_00_3f, dec_40_7f, dec_80_bf, dec_c0_ff;
    v60_decode_t dec;   // active decode bundle (selected by ibuf[0][7:6])

    v60_decode_00_3f u_dec_00_3f (
        .ibuf     (ibuf),
        .reg_file (reg_file),
        .f_z      (f_z),
        .f_s      (f_s),
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_DECODE: begin
                    ...
                    // ----------------------------------------------------------
                    // Apply decode bundle from combinational sub-modules.
                    // The active bundle (dec) is muxed by ibuf[0][7:6].
                    // Each field has an enable bit; only set fields are written.
                    // ----------------------------------------------------------

                    // Next FSM state
                    state <= state_t'(dec.next_state);

                    // S_EXECUTE control fields (always latched; valid when going to S_EXECUTE)
                    op_alu_op       <= dec.op_alu_op;
                    op_size         <= dec.op_size;
                    op_has_am2      <= dec.op_has_am2;
                    op_update_flags <= dec.op_update_flags;
                    op_is_branch    <= dec.op_is_branch;
                    op_is_single_am <= dec.op_is_single_am;
                    op_no_am        <= dec.op_no_am;
                    op_is_ext       <= dec.op_is_ext;
                    op_ext_op       <= dec.op_ext_op;

                    // Register file write port 0
                    if (dec.rw0_en)
                        reg_file[dec.rw0_idx] <= dec.rw0_val;

                    // Register file write port 1
                    if (dec.rw1_en)
                        reg_file[dec.rw1_idx] <= dec.rw1_val;

                    // PC update
                    if (dec.pc_en)
                        reg_file[32] <= dec.pc_val;

                    // PSW flags update
                    if (dec.flg_update) begin
                        f_z  <= dec.flg_z;
                        f_s  <= dec.flg_s;
                        f_ov <= dec.flg_ov;
                        f_cy <= dec.flg_cy;
                    ...
                    // PSW register direct write (e.g. UPDPSWW)
                    if (dec.psw_reg_en)
                        reg_file[33] <= dec.psw_reg_val;

                    // Stack/call helper registers
                    if (dec.stk_val_en)        stk_val         <= dec.stk_val_v;
                    if (dec.stk_size_en)       stk_size        <= dec.stk_size_v;
                    if (dec.stk_ret_pc_en)     stk_ret_pc      <= dec.stk_ret_pc_v;
                    if (dec.stk_jump_target_en) stk_jump_target <= dec.stk_jump_target_v;
                    if (dec.stk_dst_reg_en)    stk_dst_reg     <= dec.stk_dst_reg_v;

                    // PREPARE/DISPOSE frame size
                    if (dec.prep_frame_size_en) prep_frame_size <= dec.prep_frame_size_v;

                    // PUSHM/POPM
                    if (dec.pm_mask_en)    pm_mask    <= dec.pm_mask_v;
                    if (dec.pm_idx_en)     pm_idx     <= dec.pm_idx_v;
                    if (dec.pm_is_popm_en) pm_is_popm <= dec.pm_is_popm_v;

                    // MOVCUH scratch
                    if (dec.movcuh_en) begin
                        movcuh_src <= dec.movcuh_src_v;
                        movcuh_dst <= dec.movcuh_dst_v;
                        movcuh_cnt <= dec.movcuh_cnt_v;
                    end

                    // LDTASK scratch + bus control
                    if (dec.ldtask_ptr_en) ldtask_ptr <= dec.ldtask_ptr_v;
                    if (dec.bus_en) begin
                        bus_addr_r <= dec.bus_addr_v;
                        bus_as_r   <= dec.bus_as_v;
                        bus_rw_r   <= dec.bus_rw_v;
                        bus_ds_r   <= dec.bus_ds_v;
                    end
