...
`ifdef VERILATOR
            // Watchdog disabled in simulation — counter runs but never triggers reset.
            // This prevents the sim from locking up before the CPU has a chance to kick it.
...
`endif
    ...
    // vblank_irq: wire to the irq_vblank signal generated by Gate 2's vblank edge detector.
    // (Rule 11: cleared on IACK in tb_top.sv — not self-clearing here)
    assign vblank_irq = irq_vblank;
    assign hblank_irq = 1'b0;  // HBlank not implemented yet
