#!/usr/bin/env python3
"""
generate_gate3.py — Kaneko16 Gate 3 test vector generator.
...
Produces gate3_vectors.jsonl covering:
  ...
  10. Priority field preserved in display_list (non-zero priority sprite)
...
Vector operations (processed by tb_gate3.cpp):
  ...
  write_sram  addr, data   — write sprite RAM word (word index, 8 words/sprite)
...
Kaneko16 sprite RAM word layout (word index = sprite_idx*8 + word_offset):
...
from gate3_model import SpriteROM, Kaneko16SpriteEntry, Kaneko16SpriteRasterizer
...
def make_sprite_words(y: int, x: int, tile_num: int,
                      flip_x: bool, flip_y: bool,
                      palette: int, priority: int, size: int):
    """
    Return (w0, w1, w2, w3) for one Kaneko16 sprite RAM entry.
...
def generate():
    ...
    def null_sprite(slot: int):
        """Write inactive sentinel to sprite slot (8 words per slot)."""
        base = slot * 8
    ...
    def write_sprite(slot: int, y: int, x: int, tile_num: int,
                     flip_x: bool, flip_y: bool,
                     palette: int, priority: int, size: int):
        w0, w1, w2, w3 = make_sprite_words(
            y, x, tile_num, flip_x, flip_y, palette, priority, size)
        base = slot * 8
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=20, x=10, tile_num=1,
                 flip_x=False, flip_y=False, palette=3, priority=0, size=0)

    vblank_scan()

    spr = Kaneko16SpriteEntry(x=10, y=20, tile_num=1, palette=3, size=0, valid=True)
    dl  = [spr] + [Kaneko16SpriteEntry() for _ in range(255)]

    # Scanline 20 — top row of sprite (Y hit)
    scan_line(20)
    buf = rast.render_scanline(dl, 1, current_y=20)

    check_spr(9,  0, 0)                              # just before sprite
    for x in [10, 15, 20, 25]:
        p = buf[x]
        check_spr(x, int(p.valid), p.color)
    check_spr(26, 0, 0)                              # just after sprite
    ...
    # Scanline 36 — just past sprite bottom → no sprite (Y miss)
    scan_line(36)
    check_spr(10, 0, 0)

    # Scanline 19 — just above sprite → no sprite (Y miss)
    scan_line(19)
    check_spr(10, 0, 0)
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=10, x=0, tile_num=2,
                 flip_x=False, flip_y=False, palette=7, priority=0, size=0)

    vblank_scan()
    scan_line(10)

    spr2 = Kaneko16SpriteEntry(x=0, y=10, tile_num=2, palette=7, size=0, valid=True)
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=0, x=0, tile_num=3,
                 flip_x=True, flip_y=False, palette=1, priority=0, size=0)

    vblank_scan()
    scan_line(0)

    spr3 = Kaneko16SpriteEntry(x=0, y=0, tile_num=3, palette=1, size=0, flip_x=True, valid=True)
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=0, x=0, tile_num=4,
                 flip_x=False, flip_y=True, palette=2, priority=0, size=0)

    vblank_scan()

    spr4 = Kaneko16SpriteEntry(x=0, y=0, tile_num=4, palette=2, size=0, flip_y=True, valid=True)
    ...
    # tile_num=10: [10]=TL(nybble 1), [11]=TR(2), [12]=BL(3), [13]=BR(4)
    # ══════════════════════════════════════════════════════════════════════════
    reset()
    rom  = SpriteROM()
    rast = Kaneko16SpriteRasterizer(rom)
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=0, x=0, tile_num=10,
                 flip_x=False, flip_y=False, palette=5, priority=0, size=1)

    vblank_scan()

    spr5 = Kaneko16SpriteEntry(x=0, y=0, tile_num=10, palette=5, size=1, valid=True)
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=0, x=0, tile_num=20,
                 flip_x=False, flip_y=False, palette=1, priority=0, size=0)
    write_sprite(1, y=0, x=8, tile_num=21,
                 flip_x=False, flip_y=False, palette=2, priority=0, size=0)

    vblank_scan()
    scan_line(0)

    spr6a = Kaneko16SpriteEntry(x=0, y=0, tile_num=20, palette=1, size=0, valid=True)
    spr6b = Kaneko16SpriteEntry(x=8, y=0, tile_num=21, palette=2, size=0, valid=True)
    dl6   = [spr6a, spr6b] + [Kaneko16SpriteEntry() for _ in range(254)]
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=0, x=304, tile_num=30,
                 flip_x=False, flip_y=False, palette=6, priority=0, size=0)

    vblank_scan()
    scan_line(0)

    spr7 = Kaneko16SpriteEntry(x=304, y=0, tile_num=30, palette=6, size=0, valid=True)
    dl7  = [spr7] + [Kaneko16SpriteEntry() for _ in range(255)]
    ...
    # Base tile=40; column tc has nybble (tc+1): col0=1, col1=2, col2=3, col3=4
    # ══════════════════════════════════════════════════════════════════════════
    reset()
    rom  = SpriteROM()
    rast = Kaneko16SpriteRasterizer(rom)
    ...
    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=0, x=0, tile_num=40,
                 flip_x=False, flip_y=False, palette=9, priority=0, size=2)

    vblank_scan()
    scan_line(0)

    spr8 = Kaneko16SpriteEntry(x=0, y=0, tile_num=40, palette=9, size=2, valid=True)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # Scenario 9: Non-zero priority field (just checks data flows correctly)
    # Sprite at x=50, y=50, tile=60, palette=0xA, priority=7
    # ══════════════════════════════════════════════════════════════════════════
    reset()
    rom  = SpriteROM()
    rast = Kaneko16SpriteRasterizer(rom)

    load_solid_tile(rom, 60, 0xE)

    for s in range(32):
        null_sprite(s)
    write_sprite(0, y=50, x=50, tile_num=60,
                 flip_x=False, flip_y=False, palette=0xA, priority=7, size=0)

    vblank_scan()
    scan_line(50)

    spr9 = Kaneko16SpriteEntry(x=50, y=50, tile_num=60, palette=0xA,
                                size=0, priority=7, valid=True)
    dl9  = [spr9] + [Kaneko16SpriteEntry() for _ in range(255)]
    buf9 = rast.render_scanline(dl9, 1, current_y=50)

    check_spr(50, int(buf9[50].valid), buf9[50].color)  # {A,E}=0xAE
    check_spr(65, int(buf9[65].valid), buf9[65].color)  # last pixel
    chk({"op": "check_spr", "x": 49, "exp_valid": 0, "exp_color": 0})
