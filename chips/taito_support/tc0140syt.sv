...
wire bank_req = A[15:14] == 2'b01;
assign ROMCS0n = bank_req ? rom_bank[2] : A[15] | A[14];
assign ROMCS1n = ~(bank_req & rom_bank[2]);
    ...
    if (sdr_req == sdr_ack) begin
        if (request_active == 2) begin
            ...
            if (cha_request_pending) begin
                ...
                request_active <= 1;
                sdr_address <= ADPCMA_ROM_BASE + { 3'd0, cha_addr[23:1], 1'b0 };
                sdr_req <= ~sdr_req;
            end else if (chb_request_pending) begin
                chb_request_pending <= 0;
                request_active <= 2;
                sdr_address <= ADPCMB_ROM_BASE + { 3'd0, chb_addr[23:1], 1'b0 };
                sdr_req <= ~sdr_req;
            end
        end
    end
