...
<<<<<<< HEAD
                ...
                static uint32_t last_bc_addrs[16] = {};
                static int      last_bc_rw[16]    = {};
                static int      last_bc_head       = 0;

                if (!prev_asn && asn) {
                    bus_cycles++;
                    as_low_since = 0;  // reset stall counter on cycle completion
                }
                if (!asn && prev_asn) {
                    // New bus cycle starting
                    as_low_since = iter;
                    stall_addr   = addr;
...
<<<<<<< HEAD
                ...
                if (!asn && as_low_since > 0 &&
                    ...
                    stall_reported = true;
                    fprintf(stderr, "\n*** CPU STALL at iter %lu (bus_cycles=%d) ***\n",
                            (unsigned long)iter, bus_cycles);
                    fprintf(stderr, "    hblank_n_in=%d  vblank_n_in=%d  hcnt=%d  vcnt=%d\n",
                            (int)top->hblank_n_in, (int)top->vblank_n_in, hcnt, vcnt);
                    fprintf(stderr, "    Stuck addr=%06X  RW=%d  dtack_n=%d  dout=%04X  din=%04X\n",
                            (uint32_t)(((uint32_t)top->dbg_cpu_addr << 1) & 0xFFFFFF),
                            (int)top->dbg_cpu_rw,
                            (int)top->dbg_cpu_dtack_n,
                            (unsigned)(top->dbg_cpu_dout & 0xFFFF),
                            (unsigned)(top->dbg_cpu_din  & 0xFFFF));
                    // Decode which chip-select is expected at this address
                    uint32_t sa = ((uint32_t)top->dbg_cpu_addr << 1) & 0xFFFFFF;
                    fprintf(stderr, "    Address decode:\n");
                    if (sa < 0x080000)
                        fprintf(stderr, "      prog_rom_cs (0x000000-0x07FFFF)\n");
                    if (sa >= 0x200000 && sa <= 0x201FFF)
                        fprintf(stderr, "      dar_cs TC0260DAR palette (0x200000-0x201FFF)\n");
                    if (sa >= 0x400000 && sa <= 0x47FFFF)
                        fprintf(stderr, "      vcu_cs TC0180VCU (0x400000-0x47FFFF)\n");
                    if (sa >= 0x600000 && sa <= 0x607FFF)
                        fprintf(stderr, "      wram_cs (0x600000-0x607FFF)\n");
                    if (sa >= 0x800000 && sa <= 0x800003)
                        fprintf(stderr, "      syt_mcs TC0140SYT (0x800000-0x800003)\n");
                    if (sa >= 0xA00000 && sa <= 0xA0001F)
                        fprintf(stderr, "      ioc_cs TC0220IOC (0xA00000-0xA0001F)\n");
                    if (sa >= 0x080000 && sa < 0x200000)
                        fprintf(stderr, "      (OPEN BUS — no chip, DTACK never fires)\n");
                    if (sa > 0x201FFF && sa < 0x400000)
                        fprintf(stderr, "      (OPEN BUS — no chip, DTACK never fires)\n");
                    if (sa > 0x47FFFF && sa < 0x600000)
                        fprintf(stderr, "      (OPEN BUS — no chip, DTACK never fires)\n");
                    if (sa > 0x607FFF && sa < 0x800000)
                        fprintf(stderr, "      (OPEN BUS — no chip, DTACK never fires)\n");
                    if (sa > 0x800003 && sa < 0xA00000)
                        fprintf(stderr, "      (OPEN BUS — no chip, DTACK never fires)\n");
                    if (sa > 0xA0001F)
                        fprintf(stderr, "      (OPEN BUS — no chip, DTACK never fires)\n");
                    // Print last 16 bus cycles
                    fprintf(stderr, "    Last bus cycles (newest last):\n");
                    for (int ii = 0; ii < 16; ii++) {
                        int idx = (last_bc_head - 16 + ii + 256) % 16;
                        fprintf(stderr, "      [%d] addr=%06X RW=%d\n",
                                ii, last_bc_addrs[idx], last_bc_rw[idx]);
                    }
                }
