...
//   CH2  gfx read        — GFX (sprite) ROM (16-bit, toggle-handshake)
//   CH3  adpcm read      — BG tile ROM (16-bit, toggle-handshake)
//   CH4  snd read        — ADPCM (OKI M6295) ROM (16-bit, toggle-handshake)
//
...
module sdram_b (
    ...
    // CH3: ADPCM ROM read (toggle handshake, clk_sys domain) — BG tile ROM
    input  logic [26:0] adpcm_addr,
    input  logic        adpcm_req,
    output logic [15:0] adpcm_data,
    output logic        adpcm_ack,

    // CH4: SND ROM read (toggle handshake, clk_sys domain) — OKI M6295 ADPCM samples
    input  logic [26:0] snd_addr,
    input  logic        snd_req,
...
logic  [2:0]         ch_sel;     // active channel (0=ioctl,1=cpu,2=gfx,3=adpcm/bg,4=snd/oki)
...
logic  [1:0]         cpu_req_sync,   gfx_req_sync,   adpcm_req_sync,  snd_req_sync;
logic                cpu_req_clk,    gfx_req_clk,    adpcm_req_clk,   snd_req_clk;
logic                cpu_req_prev,   gfx_req_prev,   adpcm_req_prev,  snd_req_prev;
logic                cpu_req_pend,   gfx_req_pend,   adpcm_req_pend,  snd_req_pend; // pending in clk domain

// Latched addresses (captured when req edge detected)
logic [26:0]         cpu_addr_lat,   gfx_addr_lat,   adpcm_addr_lat,  snd_addr_lat;

// Read data registers
logic [15:0]         cpu_data_r,     gfx_data_r,     adpcm_data_r,    snd_data_r;
logic                cpu_ack_r,      gfx_ack_r,       adpcm_ack_r,    snd_ack_r;
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        snd_req_sync   <= {snd_req_sync[0],    snd_req};
    end
end
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (snd_req_clk != snd_req_prev) begin
            snd_req_pend  <= 1'b1;
    ...
    if (!rst_n) begin
        ...
        if (state == S_INIT_REF1 || state == S_INIT_REF2 ||
            (state == S_IDLE && ref_req && wr_fifo_empty &&
             !cpu_req_pend && !gfx_req_pend && !adpcm_req_pend && !snd_req_pend)) begin
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (state)
            ...
            //   Priority: refresh > CH0 write > CH1 > CH2 > CH3
            // -----------------------------------------------------------------
            S_IDLE: begin
                if (ref_req) begin
                ...
                else if (snd_req_pend) begin
                    row_addr    <= snd_addr_lat[25:13];
                    col_addr    <= snd_addr_lat[8:0];
                    bank_sel    <= snd_addr_lat[10:9];
                    ch_sel      <= 3'd4;
                    doing_write <= 1'b0;
                    state       <= S_ACT;
                end
            ...
            S_READ_W: begin
                ...
                if (wait_cnt == tRC_CYCLES - 3) begin
                    ...
                    case (ch_sel)
                        ...
                        3'd4: begin snd_data_r  <= SDRAM_DQ; snd_ack_r  <= snd_req_clk;  end
...
assign snd_data  = snd_data_r;
assign snd_ack   = snd_ack_r;
