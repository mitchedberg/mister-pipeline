...
module nmk16 #(
    ...
    output logic [15:0]      bg_pix_color,  // [15:8]=layer1, [7:0]=layer0: {palette[3:0], index[3:0]}
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (~rst_n) begin
            ...
            // Update only the layer just processed (g4s1_layer).
            // Explicit if/else instead of for-loop — avoids Quartus 17 Error 10028
            // (<auto-generated> multi-driver on packed output port slices).
            if (g4s1_layer == 1'b0) begin
                bg_pix_valid[0]    <= (g4s2_nibble != 4'h0);
                bg_pix_color[7:0]  <= {g4s1_palette, g4s2_nibble};
                bg_pix_priority[0] <= 1'b0;
            end else begin
                bg_pix_valid[1]    <= (g4s2_nibble != 4'h0);
                bg_pix_color[15:8] <= {g4s1_palette, g4s2_nibble};
