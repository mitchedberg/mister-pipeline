...
| `factory/DEBUG_INCIDENTS.md` | Structured incident log workflow | Before debugging or documenting any issue |
...
3b. **Write debug incidents, including failed paths.** Every meaningful hypothesis tested on a
    bug goes into `.shared/debug_incidents.jsonl` via the factory logging script. Capture:
    assumptions, attempted fix, evidence, outcome, and whether the path was a dead end.
...
### Hardware Feedback Gate (new, between gate4 and gate7)

There is a practical gate between "RBF exists" and "human tests hardware":

```
gate4.5: Remote MiSTer feedback loop
  - deploy RBF/MRA/ROMs remotely
  - launch core remotely
  - capture real screenshots remotely
  - inspect MiSTer process/file state remotely
  - only then ask for or perform physical hardware intervention
```

This gate exists to keep the hardware loop measured in seconds, not 30-minute synths plus
"yes/no" human symptom reports. Use the factory harness in `factory/` before asking a human to
walk to the DE-10.
...
Use **git worktrees** (`Codex --worktree <name>`) for file isolation between sessions.

### Before starting work
1. Read ALL mandatory files listed at top of this document
2. Query `.shared/debug_incidents.jsonl` for the relevant core/chip before starting a fresh debug loop
3. Check `.shared/task_queue.md` for your next task
4. Check `.shared/agent_comms.md` for messages to you
5. Check `.shared/failure_catalog.md` for known issues
6. Claim a task, do the work, report findings
...
## Upstream Bug Prevention Ladder

When a bug survives into a 30-minute Quartus loop, assume a missing upstream gate.
Before another full-system synthesis, ask which of these should have caught it:

1. **Pattern pass:** Does MAME or community code already show the correct structure?
   Shared return paths, SDRAM arbitration, wrapper timing, and CPU phase cadence should all be
   copied from proven patterns, not improvised.

2. **Static pass:** Can `check_rtl.sh` catch the class of bug?
   If the answer is yes, add a rule immediately after the first real incident.

3. **Hostile sim pass:** Is the sim too friendly?
   Fixed-latency or zero-contention sims hide real hardware bugs. Add variable-latency,
   owner/ack, and protocol assertions before blaming MiSTer.

4. **Standalone synth pass:** Can the subsystem be isolated?
   If a chip-level or wrapper-level harness could catch it in 5-15 minutes, do not spend a
   30-90 minute full-core synth to discover it.

5. **Remote hardware pass:** Can the live feedback harness reduce ambiguity?
   Deploy, launch, screenshot, and inspect remotely before asking for a human report.

### Mandatory postmortem after each new hardware-only bug

For every new bug that first appears on MiSTer hardware:
- add a failure-catalog entry
- add a structured incident entry with assumptions, attempts, and outcome
- decide whether it belongs in `COMMUNITY_PATTERNS.md`, `GUARDRAILS.md`, or `check_rtl.sh`
- decide whether the remote feedback harness should expose a new signal or action
- do not leave it as tribal knowledge in chat history
